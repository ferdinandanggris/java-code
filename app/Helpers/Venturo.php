...
class Venturo
{
     ...
     * @return object
     */
    private static function loadPdf(
        $filePath,
        $data,
        $title = 'no-title.pdf',
        $paperSize = ['paper' => 'a4', 'orientation' => 'landscape']
    ) {
     ...
     * @return void
     */
    public static function PdfView(
        $filePath,
        $data,
        $title = 'no-title.pdf',
        $paperSize = ['paper' => 'a4', 'orientation' => 'potrait']
    ) {
        return self::loadPdf($filePath, $data, $title, $paperSize)->stream(
            $title
        );
    }
    ...
    public static function PdfDownload(
        ...
        return self::loadPdf($filePath, $data, $title, $paperSize)->download(
            $title
        );
    }
